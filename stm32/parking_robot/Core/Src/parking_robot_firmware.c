...
static bool ErrorIsRecoverableRejection(const char *code)
{
    return strcmp(code, "LIFT_WHILE_MOVING") == 0 ||
           ...
           /* A damaged startup frame is a communication-session failure, not
            * proof of a persistent motor/sensor fault.  It must still stop
            * every wheel immediately, but a later valid HELLO may establish
            * a clean session without requiring a power cycle. */
