...
- 확인됨: STM32 수동 주행, 엔코더, 서보, 초음파 제어와 로봇별 방향·서보 한계. Front(`robot-2`)의 ROS 2 bridge, 잭업 폐루프 3축 주행, 무하중 바닥 저속 키보드 주행. 2026-08-22에는 Ramp 15를 적용해 전진·회전·횡이동을 다시 검증하고 Raspberry Pi에서 STM32를 직접 기록·검증하는 절차를 확인했다. 2026-08-25에는 Rear(`robot-1`) 교체 Nucleo에 Rear 펌웨어를 기록·읽기 검증하고 정지 UART와 400/2600µs 서보 프로파일을 확인했다. 2026-08-26에는 사용자가 전날 `robot-1`의 정상 단독 주행을 확인했다고 보고해 RR 하네스 미복구 차단 조건은 해제했다.
- 통합 필요: `robot-1`의 현 배선 상태를 유지하고 협동 시험 직전 네 바퀴 엔코더·방향을 잭업으로 재확인한다. 이후 Jetson 비전·전역 경로계획, ArUco 상대 측위, 두 로봇 강체 동기 주행과 차량 하중 시험이 남아 있다. `robot-1`의 기존 Nucleo는 소손·격리했고 교체품은 장착했다.
