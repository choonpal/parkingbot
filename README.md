...
기준 pregrip 소프트웨어 통합과 clean build는 통과했다. Front와 Rear를 각각
수동 왕복해 초음파-그리퍼 X offset 네 값 `0.0m`를 약 ±0.02m 현장 오차로
확인했다. 하지만 세 장비 동일 SHA 배포와 단계별 실기 gate가 남아 있어 차량
하부 자동 진입은 **NO-GO**다. 정확한 현재 판정은
