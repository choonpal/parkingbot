...
ISOLATED_PRODUCTION_TOPICS = (
    ...
    '/rear/wheel_scan_reset', '/rear/ultrasonic_ready', '/rear/ultrasonic_left',
...
def test_ultrasonic_edge_dds_inputs_produce_rear_axle_target():
    ...
    try:
        ...
        pub_ultrasonic_ready = feeder.create_publisher(
            Bool, '/rear/ultrasonic_ready', STATE_LATEST_QOS)
        ...
        assert _spin_until(
            ...
            lambda: (pub_state.get_subscription_count() == 1 and
                     ...
                     pub_ultrasonic_ready.get_subscription_count() == 1 and
        ...
        def wheel_target_received():
            ...
            if phase < len(phases) and phase_cycles == 0:
                ...
                stamp_ns = feeder.get_clock().now().nanoseconds
                if kind == 'ready':
                    pub_ultrasonic_ready.publish(Bool(data=True))
                elif kind == 'state_target':
