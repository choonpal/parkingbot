#!/usr/bin/env python3
"""ROS 토픽으로 받는 듀얼 CCTV 타일격자 Homography 등록 도구.

원본(dual_tile_homography_gui.py)은 ``/dev/video*``를 직접 열고 npz로
undistort까지 했다. 이 버전은 **이미 왜곡 보정된 ROS 토픽**을 구독한다.

    /cctv0/image_rect  <- cctv_rectify_node 가 발행 (이미 undistort 됨)
    /cctv2/image_rect

이렇게 바꾼 이유는 셋이다.

  1. 카메라는 한 프로세스만 열 수 있다. 원본 도구를 쓰려면 ROS 카메라
     노드를 껐다가 등록이 끝나면 다시 켜야 했다.
  2. 런타임이 실제로 쓰는 영상과 **같은 영상**에서 등록해야 한다. 도구가
     자체 undistort를 하면 미묘하게 다른 영상 위에서 H를 만들게 된다.
  3. 등록 중에도 ``camera_preview``나 ``rqt_image_view``를 함께 띄워
     확인할 수 있다.

토픽이 이미 rectified이므로 **이 노드는 undistort를 하지 않는다.**
``/cctv*/image_raw``를 실수로 지정하면 보정 안 된 영상 위에서 H가
만들어지므로 주의한다.

기능은 원본과 같다.

  * 40 cm 바닥 타일 번호 (i,j) -> world X,Y 자동 계산
  * 카메라별 Homography 계산 (5점 이상이면 RANSAC)
  * 겹침 영역 같은 타일 꼭짓점의 cam0/cam2 좌표 차이 검증
  * CAM0 + CAM2 합성 미리보기 (청록/빨강)
  * 런타임용 homography_cam{0,2}_rectified.npy 저장
  * ``cam2_only`` 모드: 기존 CAM0 H를 잠그고 CAM2만 재등록

실행
----
    # 터미널 1 — 카메라와 왜곡보정만 (YOLO/merge 없이)
...
브라우저에서 ``http://<젯슨IP>:5006/``.
VSCode 원격이면 PORTS 탭에서 5006을 forward 한다.
"""
from __future__ import annotations

import hashlib, json, math, shutil, socket, threading, time
...
class TopicCamera:
    """ROS 이미지 토픽 하나를 받아 최신 프레임만 들고 있는다.

    원본 ``Camera``와 인터페이스(``get()``, ``error``, ``label``)를 맞춰
    나머지 코드를 그대로 쓴다. 다른 점은 두 가지다.

      * ``cv2.VideoCapture``를 열지 않는다 -> ROS 카메라 노드와 충돌 없음
      * **undistort를 하지 않는다** -> 토픽이 이미 rectified이기 때문
    """

    def __init__(self, label, topic, bridge, expected_size=None,
                 stale_after_s=2.0):
        self.label = label
        self.topic = topic
        self.bridge = bridge
    ...
    def callback(self, message):
        try:
            ...
            with self.lock:
                self.error = f'이미지 변환 실패: {exc}'
            return
        size_error = ''
        if self.expected_size is not None:
            width, height = self.expected_size
            if width and height and (frame.shape[1] != width
                                     or frame.shape[0] != height):
                # 크기가 달라도 버리지 않는다. 등록은 실제 들어온 영상
                # 위에서 하는 것이 맞고, 불일치는 경고로만 알린다.
                size_error = (f'해상도 {frame.shape[1]}x{frame.shape[0]} != '
                              f'설정 {width}x{height}')
        with self.lock:
            self.latest = frame
    ...
    def get(self):
        with self.lock:
            if self.latest is None:
                return None
            if time.monotonic() - self.stamp > self.stale_after_s:
                self.error = f'{self.topic} 수신 끊김'
                return None
...
class Tool:
    def __init__(self, a, node=None):
        ...
        # 원본은 여기서 640x480이 아니면 예외를 던졌다. 이제 영상이 ROS
        # 토픽으로 오므로 실제 수신 해상도로 판단해야 한다. 크기가 다르면
        # TopicCamera가 경고 문자열만 남기고 등록은 계속 진행한다.
        self.bridge = CvBridge()
        self.cams = {
            'cam0': TopicCamera('cam0', a.cam0_topic, self.bridge,
                                (self.w, self.h)),
            'cam2': TopicCamera('cam2', a.cam2_topic, self.bridge,
                                (self.w, self.h)),
        }
        if node is not None:
            for camera in self.cams.values():
                node.create_subscription(
                    Image, camera.topic,
                    lambda msg, c=camera: c.callback(msg),
                    qos_profile_sensor_data)
                node.get_logger().info(
                    f'{camera.label} <- {camera.topic}')
        self.lock = threading.RLock()
    ...
    def _load_existing_outputs(self):
        ...
        for cam in ('cam0', 'cam2'):
            ...
            if snapshot_path.is_file():
                ...
                # 해상도가 달라도 불러온다. 등록은 그 영상 위에서 하므로
                # 일관성만 지켜지면 되고, 크기 불일치는 화면에서 보인다.
    ...
    def web(self):
        ...
        return app
    def start_web(self):
        """Flask를 데몬 스레드로 띄운다. 메인 스레드는 rclpy.spin 이 쓴다."""
        ip = 'JETSON_IP'
        try:
        ...
        thread = threading.Thread(
            ...
            name='tile-homography-web', daemon=True)
        thread.start()
        self._web_thread = thread
        banner = [
            '=' * 70,
            'Dual CCTV Tile Homography (ROS 토픽 입력)',
            f'Mode   : {"CAM2 ONLY (CAM0 LOCKED)" if self.cam2_only else "CAM0 + CAM2"}',
            f'입력   : {self.cams["cam0"].topic} , {self.cams["cam2"].topic}',
            f'Jetson : http://127.0.0.1:{self.port}',
            f'다른 PC: http://{ip}:{self.port}',
            f'VSCode : PORTS 탭에서 {self.port} forward 후 http://localhost:{self.port}',
            f'Output : {self.out}',
            '=' * 70,
        ]
        return '\n'.join(banner)
...
class TileHomographyNode(Node):
    """ROS 파라미터를 받아 Tool을 구동하는 얇은 래퍼."""

    def __init__(self):
        super().__init__('tile_homography_node')
        base = Path(__file__).resolve().parent
        self.declare_parameter('cam0_topic', '/cctv0/image_rect')
        self.declare_parameter('cam2_topic', '/cctv2/image_rect')
        self.declare_parameter('port', 5006)
        self.declare_parameter(
            'output_dir', '~/.ros/adaptive_valet_bot')
        # 0이면 수신 해상도를 그대로 쓴다(검사 안 함).
        self.declare_parameter('width', 640)
        self.declare_parameter('height', 480)
        self.declare_parameter('map_width', 4.40)
        self.declare_parameter('map_height', 3.83)
        self.declare_parameter('preview_ppm', 100)
        self.declare_parameter('ransac_threshold_m', 0.03)
        self.declare_parameter('cam2_only', False)

        def value(name):
            return self.get_parameter(name).value

        args = SimpleNamespace(
            cam0_topic=str(value('cam0_topic')),
            cam2_topic=str(value('cam2_topic')),
            port=int(value('port')),
            output_dir=str(value('output_dir')),
            width=int(value('width')),
            height=int(value('height')),
            map_width=float(value('map_width')),
            map_height=float(value('map_height')),
            preview_ppm=int(value('preview_ppm')),
            ransac_threshold_m=float(value('ransac_threshold_m')),
            cam2_only=bool(value('cam2_only')),
        )
        self.tool = Tool(args, node=self)
        for line in self.tool.start_web().split('\n'):
...
def self_test():
    """카메라 없이 돌릴 수 있는 기하 자체검사.

    npz는 더 이상 이 도구가 쓰지 않으므로(토픽이 이미 rectified) 검사에서도
    뺐다. 확인하는 것은 "타일 격자 -> Homography -> 역변환"이 왕복하는가와
    저장/로드가 정상인가 둘이다.
    """
    world = np.array([[0, 0], [1.6, 0], [3.2, 0],
                      [0, 1.6], [1.6, 1.6], [3.2, 1.6],
                      [0, 3.2], [1.6, 3.2], [3.2, 3.2]], np.float64)
    forward = np.array([[120, 7, 80], [4, 90, 50], [.01, .02, 1]], np.float64)
    pixels = cv2.perspectiveTransform(
        world.reshape(-1, 1, 2), forward).reshape(-1, 2)
    matrix, _ = cv2.findHomography(pixels, world, 0)
    restored = cv2.perspectiveTransform(
        pixels.reshape(-1, 1, 2), matrix).reshape(-1, 2)
    residual = np.linalg.norm(restored - world, axis=1).max()
    assert residual < 1e-5, residual
    refs = [{'pixel': [float(p[0]), float(p[1])],
             'world': [float(w[0]), float(w[1])]}
            for p, w in zip(pixels, world)]
    src, dst = validate_refs(refs)
    assert src.shape == (9, 2) and dst.shape == (9, 2)
    print(f'SELF TEST OK (왕복 오차 {residual:.2e} m)')


def main(args=None):
    rclpy.init(args=args)
    node = TileHomographyNode()
    try:
        rclpy.spin(node)
