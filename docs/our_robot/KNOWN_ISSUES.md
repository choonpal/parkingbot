...
아래 기능 문제는 2026-08-29까지의 관측 이력이다. 현재 production entry point는
`rigid_body_sync_vehicle_global_node`이며, 과거 키보드 시험의 leader-follower 요구와
자동 가상 강체 제어는 구분한다. 제공 영상에서 주행이 보인다는 이유만으로
기존 timeout·yaw·timestamp 문제를 일괄 해결 처리하지 않는다.
현재 시연 범위와 새 배포본 검증은 [통합 상태](../CURRENT_INTEGRATION_STATUS.md)를 따른다.

2026-09-04 문서 불일치 수정:

- README의 `1.11.0`을 package의 `1.11.3`과 통일했다.
- 현재 launch의 `stop_after_align=false`와 과거 정렬 전용 시험의 `true`를 구분했다.
- 천장 카메라 C920/C922, Rear OV2710 ID0 역할과 Front 보조 카메라를 반영했다.
- “운반 범위 밖”이라는 과거 감사 범위와 실제 축소형 시연 증거를 구분했다.
