...
> 2026-08-23 등록 레이아웃이 아래 초기 예시 좌표를 대체한다. 현재 권위값은
> `parking_layout.yaml`의 Front `(3.6, 0.6, 180°)`, Rear
> `(3.6, 0.2, 180°)`이며 실차 launch도 같은 값을 사용한다.

기본 적재 footprint는
`max(0.90, 0.70+0.565)+2×0.06 = 1.385m` ×
`max(0.35, 0.275)+2×0.06 = 0.47m`다. 기존 `6m × 4m` 맵 자체는 사용할 수
있다. 아래 `(2.3, 0.6)`, `(1.15, 0.6)`, `(0.45, 0.6)` 값은 이 문서 작성
당시의 초기 검토 배치이며 현재 실차 운용값으로 사용하지 않는다.
