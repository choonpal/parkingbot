...
`simultaneous_entry=false`인 Front-first 접근에서는 Rear가 `WAIT_FRONT_STAGED`에 freshness window보다 오래 머물 수 있다. 따라서 Fleet는 현재 retrieve가 `WAIT_LIFT`이고 source Registry record가 같은 `mission_id`의 `EXIT_RESERVED`인 동안 같은 보관 차량 자세와 제원을 기존 두 topic에 fresh timestamp로 반복 발행한다. 인양이 확인되면 재발행을 끝낸다. 이는 stale 검사를 완화하는 것이 아니라 지연 진입하는 Rear에게 같은 mission-scoped 권위값을 전달하는 방식이다.
