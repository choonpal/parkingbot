...
| 현재 통합본 원격 배포 | 미완료 | 로봇에 남은 기존 설치본을 최신 통합본으로 간주하지 않음 |
| 차량 하부 실기 모션 | **NO-GO** | 좌우 초음파-그리퍼 X offset 네 값의 실측 근거가 없음 |
...
현재 현장 설정의 다음 값은 모두 `0.0`이며, 실제 센서와 그리퍼가 같은 X선이라는
측정 기록이 없다.
...
각 값은 `gripper_x - ultrasonic_sensor_x`다. 실제로 0인지 측정해 기록하거나
